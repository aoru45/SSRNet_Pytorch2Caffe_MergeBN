    ...
    def convert(self):
        ...
        while l < length:
            ...
            if param_layers[i].type in layer_type:
                if param_layers[i+1].type == "BatchNorm":
...
def connect(input_file, output_file):
    txt = ""
    with open(input_file) as f:
        #  bottom: "x_layer1_1_scale" -> x_layer1_0_conv
        for line in f:
            if "scale" in line:
                #print(line)
                new_line = "_".join(line.split("_")[:-2])
                num = int(line.split("_")[-2])
                line = new_line + "_" + str(num -1) + "_conv\"\n"
...
if __name__ == '__main__':
    ...
    cb.convert()
    connect(args.caffe_without_bn_config_filename, "ssrnet_merge_bn_connect.prototxt")
